...
networking = lib.mkIf (config.system.nixos.label == "router-setup") {
    ...
    # Isolated bridges (no host IP - guest VMs isolated from host)
    bridges.virbr4.interfaces = [];
    bridges.virbr5.interfaces = [];

    # Firewall configuration for router mode
    firewall = {
        extraCommands = ''
            ...
            # Isolated bridges (virbr4, virbr5) - only allow router VM access
