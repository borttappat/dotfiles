...
{ config, lib, pkgs, ... }:
{
  ...
  # VFIO Configuration for Router VM PCI Passthrough
  boot.kernelParams = [ 
    "intel_iommu=on" 
    "iommu=pt" 
  ...
  networking.firewall = {
    ...
    '';
    trustedInterfaces = [ "virbr1" "virbr2" ];
  };
  ...
  systemd.services.pentest-vm-auto-autostart = {
    ...
    script = ''
      ...
      # Ensure pentest VM image exists at expected location
