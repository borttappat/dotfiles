...
def upload_and_execute(
    ssh, local_file, remote_path, save_local=True, save_remote=False
):
    try:
...
def main():
    ...
    parser.add_argument(
        "-s", "--script", required=True, help="Local script to upload and execute"
    )
    ...
    parser.add_argument(
        "-r", "--remote-path", default="/tmp", help="Remote path for script upload"
    )
    parser.add_argument(
        "-n", "--no-local-save", action="store_true", help="Don't save output locally"
    )
    parser.add_argument(
        "-sr", "--save-remote", action="store_true", help="Save output on remote system"
    )
    ...
    if ssh:
        try:
            if upload_and_execute(
                ssh,
                args.script,
                args.remote_path,
                not args.no_local_save,
                args.save_remote,
            ):
